...
def check_reg(val1,val2):                             # mov reg32 reg32 mod R/M
    ...
    if val1 in reg_ecx and val2 in reg_edi:
        return "FA"
    if val1 in reg_edx and val2 in reg_edi:
        return "FB"
...
def find_pop_opcode(list1):
    if len(list1[1])<6:
        ...
        if ord(val1[0]) not in range(48,57):
            if val1 in reg_sym16:    
                for i in range(len(reg_sym16)):
                    if reg_sym16[i]==val1:
                        val2=58+int(reg_sym16[i-1])
                        return "66"+str(val2)+"               "
            if val1 in reg_sym32:
                for i in range(len(reg_sym32)):
                    if reg_sym32[i]==val1:
                        val2=58+int(reg_sym32[i-1])
                        return str(val2)+"               "
    ...
    if len(list1[1])>6:
        ...
        if val1[0]=='w':
            sym=check_sym(val1,5)
            add=find_sym_add(sym,8)
            return "668F35"+str(add)+"    "


def append_pop(line1,list1):
    line_no=[]
    address=[]
    opcode=[]
    orignal=[]
